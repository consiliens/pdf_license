...
# Generate the PDF files from the ODT folder to run the tests.
describe PdfLicense do
...
http://creativecommons.org/licenses/by-sa/3.0/us/legalcode
  ...
  pdf_folder = File.expand_path '../../pdf', __FILE__


  # RSpec inside a block doesn't fail so store results and check after.  
  it 'should fail on no license' do
    results = []

    pdfs = PdfLicense.new "#{pdf_folder}/fail/**/*", license
    pdfs.check { |m| results.push m[0] }

    results.each { |m| m.should == false }
  end


  it 'should match on a license' do
    results = []

    pdfs = PdfLicense.new "#{pdf_folder}/ok/**/*", license
    pdfs.check { |m| results.push m[0] }

    results.each { |m| m.should == true }
  end
